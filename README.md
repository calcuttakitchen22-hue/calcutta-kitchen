...
> **Internal tools:** `/internal/*` is not linked publicly and is excluded from
> search engines and the generated sitemap, but it must be protected with
> Cloudflare Access before production use. In Cloudflare Zero Trust, create a
> self-hosted application for `calcuttakitchen.in/internal/*`, add an Allow policy
> for approved Calcutta Kitchen identities or email domains, and retain the
> default deny behavior for everyone else.
